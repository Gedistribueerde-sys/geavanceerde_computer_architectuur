...
In the CUDA execution model, threads are organized into \textbf{warps} of 32. The GPU executes instructions for each warp in a SIMT (Single Instruction, Multiple Threads) fashion. When accessing global memory, the GPU attempts to \textit{coalesce} the memory requests from all threads in a warp into the minimum number of 32-byte transactions.
...
\subsection{Results}
To measure the performance difference, we benchmarked the two kernels across 100 runs for various input sizes and thread block configurations. The input image is 960x1280 pixels, and we tested it at its original size (1x), as well as scaled up by 2x and 4x. The tables below show the average execution time in milliseconds. Figure \ref{fig:part_1} summarizes the results graphically.
...
As shown in the tables and visualized in Figure \ref{fig:memory_transfer_times}, the time required for Host-to-Device (HtoD) memory transfers increases with the matrix size (N), which is expected as more data needs to be moved. The `Host to Constant (B)` transfer is notably faster for small matrices but is constrained by the 64 KB limit of constant memory, becoming unavailable for N=192 and larger. The `Load to Shared` time, which measures the cost of moving data from global to shared memory, also scales with matrix size. This is an overhead inherent to the tiled algorithm but is compensated by the faster access to shared memory during computation.
...
\begin{itemize}
    \item For the shared memory kernel, a larger block size (\texttt{BLOCK\_SIZE=16}) is clearly superior. It allows for more data reuse within each tile, which better amortizes the cost of loading data from global memory. The smaller tiles of \texttt{BLOCK\_SIZE=4} do not seem to hold enough data to be effective.
    \item For the global memory kernel, a smaller block size (\texttt{BLOCK\_SIZE=4}) appears to yield better performance for larger matrices. This might be related to how the GPU schedules the larger number of smaller blocks, potentially leading to better occupancy and latency hiding.
\end{itemize}

\paragraph{4. Scaling with Matrix and Grid Size}
All kernel execution times increase with matrix size, which is consistent with the $\mathcal{O}(N^3)$ complexity of matrix multiplication. However, the rate of increase differs. The shared memory kernel (with \texttt{BLOCK\_SIZE=16}) scales more gracefully than the global kernel, as its effective use of shared memory mitigates the memory bandwidth bottleneck that becomes more pronounced at larger scales. The grid size, which is determined by $N$ and \texttt{BLOCK\_SIZE}, grows accordingly, providing more parallelism for the GPU to exploit.
...
\section{Conclusion}
Employing shared memory improves performance for large matrices if the tile size is chosen appropriately. Constant memory is optimal only for very small data due to size constraints. Global-only approaches, while simpler, are significantly less efficient at large scales due to limited memory bandwidth reuse.
